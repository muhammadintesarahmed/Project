...
import com.service.SpringMailSender;
...
public class CartController {
	...
	@Autowired(required=true)
	SpringMailSender sms;

	
	public SpringMailSender getSms() {
		return sms;
	}

	public void setSms(SpringMailSender sms) {
		this.sms = sms;
	}
	...
	public ModelAndView save(@SessionAttribute("signIn") SignIn si, @ModelAttribute("cart") Orders order, Main main) {
		...
		if (res == false && si == null) {
			...
			/* getCartDAO().msg(order); */
			getSms().sendMail(order);
			mav.addObject("order", new Orders());
			return mav;
		}
	}
